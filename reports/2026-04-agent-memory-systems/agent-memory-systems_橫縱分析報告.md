...
> **如果你只有 30 秒，看完這張卡就走**——後續章節是論證與細節，不看不影響你做決定。

| 你的情境 | 第一推薦 | 第二選擇 | 為什麼（一句話） |
|---|---|---|---|
| Claude 生態開發者（Code / Agent SDK） | **Anthropic Memory tool** + CLAUDE.md | + Mem0（補多 user） | tool primitive 路線、不需 external service、與 Claude 工作流原生對齊 |
| Self-host 企業（資料不能離內網） | **Mem0 OSS** 或 **Zep self-host** | Cognee（air-gapped）/ Letta self-hosted | Long Context / 託管 memory 都不可用，唯一路徑就是 framework |
| 短歷史 chatbot（< 500K tokens） | **Long Context + Prompt Cache** | Anthropic Memory tool | 不需框架就解決，便宜 10×、簡單 10× |
| 同 project coding session | **Long Context**（codebase 全塞） | CLAUDE.md + Memory tool | session 內單純，不值得上 memory pipeline |
| 跨月跨年個人偏好累積 | **Mem0** | LangMem（若已在 LangGraph） | extraction-heavy 但 read 飛快、生態整合最廣 |
| 事實會變的長期 agent（客服 / 法律 / 醫療） | **Zep / Graphiti** | Mem0g | 唯一原生支援 temporal fact validity window |
| 跨任務累積技能的 autonomous agent | **Letta** | — | 唯一原生支援「agent 永遠在跑」的 OS-style 模型 |
| 個人 PKM / 開發者 second brain | **Karpathy LLM Wiki** 模式 | Anthropic Memory tool / Obsidian + Mem0 | 編譯一次 N 次受惠、cross-references 已就位 |
| 離線 / 隱私敏感（air-gapped, GDPR） | **MemPalace** 或 **Cognee** | + Ollama 本地推論 | verbatim 守原文、純 local、MIT 授權 |
| ChatGPT C 端使用者 | **OpenAI Memory**（內建） | — | 不需 external 配置、產品層面已透明化 |

**選型前的三個 gate**：

1. **Session 累積會超過 500K tokens 嗎？** 不會 → 先試 Long Context + Cache，不要急著上 framework
2. **資料能離開內網嗎？** 不能 → 跳過所有閉源 API 與託管 memory，只看 self-host 路線
3. **你 commit 哪個 LLM 生態？** 單押 Claude → Anthropic Memory tool；多 provider → Mem0 / Zep / Letta

---

## 本報告閱讀導引

報告共 11 章，依不同身份提供建議路徑：

| 你的角色 / 目的 | 建議閱讀路徑 | 預估時間 |
|---|---|---|
| 趕時間決策者 | **30 秒選型卡** + **§0 摘要** | 3 分鐘 |
| 想知道每個框架定位 | **§2 五大流派 + 三條對照路線 + LangMem**（11 個選項全在這） | 15 分鐘 |
| 想了解設計取捨的根 | **§3 設計取捨** + **§5 認知架構視角** | 25 分鐘 |
| 要做 benchmark 評估 | **§4 Benchmark 解讀** | 10 分鐘 |
| 主管要看選型結論 | **§0** + **§7 選型決策樹**（兩層 gate + 場景速查） | 10 分鐘 |
| 採購 / 投資判斷 | **§7** + **§10 能力限制聲明** | 15 分鐘 |
| 完整深度研究 | 全文按序 | 90 分鐘 |

每章開頭都有粗體「**本節結論**」3-5 行——只看本節結論就能掌握該章重點，細節留給有興趣的讀者。
...
**三條對照路線**（不是 memory 框架本身，但會壓縮或重劃 memory 框架的市場空間）：
...
<svg viewBox="0 0 580 280" xmlns="http://www.w3.org/2000/svg">
  <text x="290" y="22" text-anchor="middle" font-size="12pt" font-weight="700" fill="#1a3d7c">11 個選項的抽象層級全景</text>

  <!-- 第三層 概念 -->
  <rect x="40" y="40" width="500" height="38" fill="#f1c40f" stroke="#b7950b" stroke-width="1" rx="4"/>
  <text x="55" y="56" font-size="9pt" font-weight="700" fill="#1a3d7c">第三層 ｜ 概念 / 範式（不是 product，是設計主張）</text>
  <text x="55" y="72" font-size="10pt" font-weight="700" fill="#5b2c6f">• Karpathy LLM Wiki</text>
  <text x="540" y="72" text-anchor="end" font-size="8pt" fill="#666" font-style="italic">需要自建落地</text>

  <!-- 第二層 service -->
  <rect x="40" y="92" width="500" height="62" fill="#85c1e9" stroke="#1a5276" stroke-width="1" rx="4"/>
  <text x="55" y="108" font-size="9pt" font-weight="700" fill="#1a3d7c">第二層 ｜ Service-layer（獨立服務／自部署或託管）</text>
  <text x="55" y="125" font-size="10pt" font-weight="700" fill="#1a5276">• Mem0   • Zep / Graphiti   • Letta   • LangMem   • MemPalace   • Cognee</text>
  <text x="55" y="145" font-size="8pt" fill="#666" font-style="italic">第三方框架／跨 LLM provider／multi-user 共享主流路徑</text>

  <!-- 第一層 tool primitive -->
  <rect x="40" y="168" width="500" height="50" fill="#48c9b0" stroke="#117a65" stroke-width="1" rx="4"/>
  <text x="55" y="184" font-size="9pt" font-weight="700" fill="#117a65">第一層 ｜ Tool-primitive（內建於 LLM，由 LLM 自己讀寫）</text>
  <text x="55" y="201" font-size="10pt" font-weight="700" fill="#117a65">• Anthropic Memory tool + Files API   • OpenAI Memory（ChatGPT）</text>
  <text x="55" y="215" font-size="8pt" fill="#666" font-style="italic">與 provider 生態綁定；最低工程介入；非通用協議</text>

  <!-- 第零層 -->
  <rect x="40" y="232" width="500" height="38" fill="#d6dbdf" stroke="#666" stroke-width="1" rx="4"/>
  <text x="55" y="248" font-size="9pt" font-weight="700" fill="#1a3d7c">第零層 ｜ 繞過記憶問題（不上 memory 框架）</text>
  <text x="55" y="264" font-size="10pt" font-weight="700" fill="#1a3d7c">• Long Context + Prompt Cache（1M tokens + 5min TTL）</text>
</svg>

*圖 0：11 個選項的抽象層級全景——選錯抽象層比選錯框架更貴。第零層適合「不需要記憶」的場景，第一層綁 Anthropic / OpenAI 生態，第二層是 multi-provider / self-host 主流，第三層是設計指引而非 product。*

最值得深思的兩條深層張力：

**張力一 — Karpathy 的三顆思想炸彈**：
...
**張力二 — Service-layer vs Tool-primitive 的設計哲學衝突**：

- **Service-layer 派**（Mem0 / Zep / Letta）：把 memory 當獨立服務，外部包圍 LLM
- **Tool-primitive 派**（Anthropic）：把 memory 當 LLM 的 tool primitive，由 LLM 自己讀寫
- 這是兩條不相容的路線，選錯框架會卡在錯的抽象層
...
> **本節結論**
> - 記憶是 2026 年 agent 真正卡住的瓶頸，不是 reasoning / tool use
> - 當前 LLM 像 anterograde amnesia 患者：權重訓練後就凍結，新對話內容無法沉澱
> - Karpathy 把 agent 元年改稱「Decade of Agents」，其中記憶是最尖銳的架構性缺陷
> - 業界四種解法（context window / Long Context+Cache / RAG / persistent memory）只有最後一種真正回答跨 session 連續性
> - 後續章節討論的 11 個選項都在做「persistent memory」這條長條的不同切法
...
<svg viewBox="0 0 580 260" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="60" y="165" font-size="9pt" font-weight="700" fill="#1a3d7c">Persistent memory（Mem0 / Zep / Letta / MemPalace / Wiki / Memory tool）</text>
</svg>

*圖 1：四種記憶解法的時間跨度覆蓋——context window 撐單次對話；Long Context + Cache 把「房間」放大到 1M tokens 並透過 cache 跨數分鐘 session 復用，但僅閉源 API；RAG 臨時拉資料不累積；persistent memory 才真正跨 session、跨月、跨年。*

Context window 即使做到 1M tokens，仍然是「房間」不是「腦」——關門就空。但 Anthropic / Google / OpenAI 推出的 prompt cache（5 分鐘 TTL）讓「房間」可以在連續 session 之間被重用，這是介於 context 與 persistent memory 之間的灰色地帶。對短歷史 chatbot / 同 project coding session（< 500K tokens），這條路比上 memory 框架簡單 10 倍且更便宜（§2.6 詳述）；但**僅限閉源 API（Claude / Gemini / GPT-4.1），自部署開源模型 1M 的落地可行性差**——這是企業 self-host 場景的關鍵限制。

RAG 是個臨時資料庫，每次重新 query 都從零開始 synthesis。**只有 persistent memory 那條長條真正回答了 Karpathy 的問題**：跨 session、跨月、跨年的連續性。

但 persistent memory 的實作差異巨大。下一章把這條長條切開來看，並涵蓋三條對照路線。
...
> **本節結論**
> - 11 個選項分四層抽象：第零層 Long Context+Cache（不上框架）／第一層 tool-primitive（Anthropic / OpenAI Memory）／第二層 service-layer（Mem0 / Zep / Letta / LangMem / MemPalace / Cognee）／第三層 概念範式（Karpathy LLM Wiki）
> - 第二層 5 大主流流派比拚的軸：verbatim ↔ extraction、flat ↔ hierarchical
> - LongMemEval 同口徑：**Zep 63.8% > Mem0 49.0%**；MemPalace 96.6% 是不同 metric，不可直接比
> - MemPalace 4 月爆紅但 benchmark 爭議大，獨立重現只有 88.9% R@10、無 reranking 跌至 60.3%
> - 第三條對照路線（Long Context / Anthropic / OpenAI）會壓縮 service-layer 的市場空間，但不會消滅它
...
<svg viewBox="0 0 580 240" xmlns="http://www.w3.org/2000/svg">
  ...
  <text x="160" y="78" text-anchor="middle" fill="#fff" font-size="11pt" font-weight="700">Main context</text>
  <text x="160" y="96" text-anchor="middle" fill="#fff" font-size="9pt">≈ RAM</text>
  <text x="160" y="115" text-anchor="middle" fill="#fff" font-size="7pt">快、小，context 內可見</text>

  <!-- External context（Disk）-->
  <rect x="340" y="50" width="160" height="80" fill="#5b2c6f" rx="6"/>
  <text x="420" y="78" text-anchor="middle" fill="#fff" font-size="11pt" font-weight="700">External context</text>
  <text x="420" y="96" text-anchor="middle" fill="#fff" font-size="9pt">≈ Disk</text>
  <text x="420" y="115" text-anchor="middle" fill="#fff" font-size="7pt">慢、大，archival storage</text>
...
2026-04-08 釋出，第一波 48 小時內衝到 22K stars，截至 2026-04-29 累積到 ~47K stars。宣稱在 LongMemEval 上達到 96.6% R@5——比 Zep 的 63.8% 高出 33 點。社群一開始震撼，幾天後批評文鋪天蓋地。
...
<svg viewBox="0 0 580 270" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- 箭頭：終點落在 box 邊緣外側 -->
  <line x1="142" y1="85" x2="168" y2="85" stroke="#1a3d7c" stroke-width="2"/>
  <polygon points="168,80 178,85 168,90" fill="#1a3d7c"/>
  <line x1="282" y1="85" x2="308" y2="85" stroke="#1a3d7c" stroke-width="2"/>
  <polygon points="308,80 318,85 308,90" fill="#1a3d7c"/>
  <line x1="422" y1="85" x2="448" y2="85" stroke="#1a3d7c" stroke-width="2"/>
  <polygon points="448,80 458,85 448,90" fill="#1a3d7c"/>
...
### §2.6 Long Context + Prompt Cache ── context-first 路線
...
<svg viewBox="0 0 580 280" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- 註腳 -->
  <text x="290" y="255" text-anchor="middle" font-size="9pt" fill="#999" font-style="italic">對 self-host 企業，Long Context 路線實質不可用；memory 框架仍是主體。</text>
...
*圖 X1：Long Context + Cache vs Memory 框架的四象限決策邊界。「自部署 + 長歷史」與「閉源 API + 長歷史」都仍需 memory 框架；「閉源 API + 短歷史」最划算路徑就是直接吃 Long Context + Cache。*
...
### §2.7 Anthropic Memory tool + Claude Skills ── tool-primitive 路線
...
### §2.8 OpenAI Memory / ChatGPT Personalization ── 市佔最大的 memory layer
...
### §2.9 LangMem ── LangChain 生態的 first-class memory

**定位**：LangMem 是 LangChain 生態（含 LangGraph）為 first-class memory 抽象提供的 module，2024 末釋出，2026 年逐步整合進 LangGraph 主流 API。LangChain 是當前最大的 agent framework 生態，LangMem 是該生態的預設 memory 方案，影響面廣，因此單列一節而非歸入次主流。
...
主菜（§2.1-§2.5）+ 三條對照路線（§2.6-§2.8）+ LangMem（§2.9）以外，2026 年活躍的次主流框架：
...
emergentmind 整理（公式形式為轉錄自 emergentmind 的整理筆記，並非標準 Ebbinghaus forgetting curve `R = exp(-t/τ)`；雙重指數形式接近 Gompertz survival function，建議讀者直接以核心觀念理解，公式具體形式以原論文為準）：
...
> **本節結論**
> - 三派儲存哲學定位：**Verbatim 守原文**（MemPalace）／**Extraction 抽事實**（Mem0 / Zep）／**Compiled-Wiki 編譯結構**（Karpathy）
> - Write-time vs Read-time 成本是所有框架的第一個分岔——Mem0 / Zep 寫貴讀快，MemPalace 寫零成本但讀靠 retrieval engine
> - RAG vs Wiki 不是替代關係：RAG 適合 large dynamic corpus，Wiki 適合 personal / project knowledge
> - 中文場景所有框架都是英文中心，**需自行投資 NLP 中介層**（embedding 換 BGE-zh、entity normalization、temporal layer）
...
<svg viewBox="0 0 580 280" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- 中央：trade-off 軸 -->
  <text x="290" y="155" text-anchor="middle" font-size="8pt" fill="#666" font-style="italic">資訊保留度</text>
  <text x="180" y="180" text-anchor="middle" font-size="8pt" fill="#666" font-style="italic">查詢速度</text>
  <text x="400" y="180" text-anchor="middle" font-size="8pt" fill="#666" font-style="italic">結構化 / 跨 session 累積</text>

  <!-- Letta 在中間 -->
  <circle cx="290" cy="170" r="6" fill="#f1c40f" stroke="#1a3d7c" stroke-width="1.5"/>
  <text x="305" y="172" font-size="8pt" fill="#1a3d7c" font-weight="700">Letta（混合）</text>
</svg>
...
所有主流框架都是英文中心。中文場景的真實落地門檻比想像的高，分四層問題拆解：
...
> **本節結論**
> - LongMemEval 是當前最佳對照基準，**僅代表「閒聊體 long-context 場景」**，對 coding agent / domain-specific / 多 agent / 中文場景代表性低
> - 跨家數字直接比是不嚴謹的——每家用不同 model + embedding + dataset，**MemPalace 96.6% 是 retrieval recall，Mem0 49.0% 是 answer correctness，本質不可比**
> - 2026 年 benchmark inflation 嚴重，每家都在自家口徑下宣稱最強
> - 採購原則：(a) 第三方獨立驗證 (b) 口徑一致 (c) 在你的場景重現
> - **在自家真實 task 上跑 ablation 永遠優於信第三方 benchmark**
...
### §4.3 LongMemEval / LoCoMo 自身的 dataset bias
...
> **本節結論**
> - 人腦記憶四元論（**Working / Episodic / Semantic / Procedural**）是當前所有框架的設計原型
> - 5 大主流框架在四類記憶的支援度差異大：MemPalace 弱在 procedural，LLM Wiki 弱在 episodic 自動化
> - Karpathy 的核心主張：當前缺一個「**System Prompt Learning**」範式——LLM 會自己寫筆記、總結教訓
> - Spreading activation（人腦聯想擴散）是 retrieval 下一條路，但生產級框架尚未原生支援
> - 三階段 consolidation（**Episodic → Semantic → In-Weights**）才是 Karpathy 真實野心，最終要把記憶烤回權重——若這成立，現有所有框架都是過渡形態
...
<svg viewBox="0 0 580 260" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- 底部對應人類 -->
  <text x="290" y="200" text-anchor="middle" font-size="9pt" fill="#666" font-style="italic">人腦對應：白天經驗 → 睡眠 hippocampus 重播 → cortex 長期儲存</text>
  <text x="290" y="220" text-anchor="middle" font-size="9pt" fill="#c3343e" font-weight="700">"I want to remove the memory... and only maintain the algorithms for thought"</text>
  <text x="290" y="238" text-anchor="middle" font-size="8pt" fill="#666">— Karpathy on Cognitive Core</text>
</svg>
...
> **本節結論**
> - Agent 系統三層堆疊：**Capability**（LLM / Cursor / Devin）→ **Orchestration**（MCP / A2A / LangGraph）→ **Memory**（Mem0 / Zep / Letta / MemPalace / Wiki）
> - 三層理論獨立、實務常重疊：Letta 越界進編排層、MCP 越界進記憶層、Karpathy LLM Wiki 同時跨三層
> - 多 agent 共享 memory 是當前最弱的環節——所有 service-layer 框架都假設單 agent / 單 user
> - 第二題（多 agent 編排研究）的核心問題：memory 該不該離開 agent 框架成為獨立 service
...
> **本節結論**
> - 兩層 gate 決策：**(1) session 規模 → 是否需要 framework**；**(2) 部署環境 → Claude 生態 or multi-provider**
> - 四條主路徑：(a) Long Context + Cache、(b) 自管 context + 輕量 memory、(c) Anthropic Memory tool 四件套、(d) Mem0 / Zep / Letta self-host
> - 第三方 memory 框架（Mem0 / Zep / Letta）的真實 niche：**self-host enterprise + 跨月跨年 + 多 agent autonomous**
> - 選 Mem0 / Zep 之前先問：Long Context、Anthropic Memory tool、OpenAI Memory **這三條路是否都不適用**
> - 細節分流（事實演變→Zep / 個人化→Mem0 / Autonomous→Letta / 離線→Cognee/MemPalace）屬第二級決策

本章把選型拆成兩層 gate + 四條主路徑。第一層 gate 決定要不要上 memory 框架，第二層 gate 決定走 Claude 生態還是 multi-provider，最後才到 Mem0 / Zep / Letta 的具體場景分流。

<div style="page-break-before: always; height: 0;"></div>

<svg viewBox="0 0 580 270" xmlns="http://www.w3.org/2000/svg">
  <text x="290" y="20" text-anchor="middle" font-size="11pt" font-weight="700" fill="#1a3d7c">選型決策樹：兩層 gate + 四條主路徑</text>

  <!-- Gate 1: session 規模 -->
  <rect x="200" y="38" width="180" height="32" fill="#1a5276" rx="4"/>
  <text x="290" y="58" text-anchor="middle" fill="#fff" font-size="10pt" font-weight="700">Gate 1：session 規模？</text>

  <!-- Gate 1 → Gate 2a / 2b -->
  <line x1="240" y1="70" x2="130" y2="92" stroke="#1a5276" stroke-width="1.5"/>
  <line x1="340" y1="70" x2="450" y2="92" stroke="#1a5276" stroke-width="1.5"/>

  <!-- Gate 2a：短歷史 + 部署環境 -->
  <rect x="20" y="92" width="220" height="32" fill="#117a65" rx="4"/>
  <text x="130" y="112" text-anchor="middle" fill="#fff" font-size="9pt" font-weight="700">Gate 2a：&lt; 500K + 閉源 API？</text>

  <!-- Gate 2b：長歷史 + 部署環境 -->
  <rect x="340" y="92" width="220" height="32" fill="#b7950b" rx="4"/>
  <text x="450" y="112" text-anchor="middle" fill="#fff" font-size="9pt" font-weight="700">Gate 2b：&gt; 500K + 部署？</text>

  <!-- Gate 2a 分支 -->
  <line x1="80" y1="124" x2="80" y2="150" stroke="#117a65" stroke-width="1.5"/>
  <line x1="180" y1="124" x2="180" y2="150" stroke="#117a65" stroke-width="1.5"/>

  <!-- Leaf 1：Long Context + Cache -->
  <rect x="15" y="150" width="130" height="50" fill="#48c9b0" rx="4"/>
  <text x="80" y="168" text-anchor="middle" fill="#fff" font-size="8pt" font-weight="700">是 → Long Context</text>
  <text x="80" y="182" text-anchor="middle" fill="#fff" font-size="8pt">+ Cache</text>
  <text x="80" y="194" text-anchor="middle" fill="#fff" font-size="7pt">（不需框架）</text>

  <!-- Leaf 2：自管 context + 輕量 memory -->
  <rect x="155" y="150" width="130" height="50" fill="#5b2c6f" rx="4"/>
  <text x="220" y="168" text-anchor="middle" fill="#fff" font-size="8pt" font-weight="700">否 → 自管 context</text>
  <text x="220" y="182" text-anchor="middle" fill="#fff" font-size="8pt">+ 輕量 memory</text>
  <text x="220" y="194" text-anchor="middle" fill="#fff" font-size="7pt">（MemPalace/Cognee）</text>

  <!-- Gate 2b 分支 -->
  <line x1="400" y1="124" x2="400" y2="150" stroke="#b7950b" stroke-width="1.5"/>
  <line x1="500" y1="124" x2="500" y2="150" stroke="#b7950b" stroke-width="1.5"/>

  <!-- Leaf 3：Claude 生態 → Memory tool 四件套 -->
  <rect x="335" y="150" width="130" height="50" fill="#1a5276" rx="4"/>
  <text x="400" y="168" text-anchor="middle" fill="#fff" font-size="8pt" font-weight="700">Claude 生態 →</text>
  <text x="400" y="182" text-anchor="middle" fill="#fff" font-size="8pt">Memory tool 四件套</text>
  <text x="400" y="194" text-anchor="middle" fill="#fff" font-size="7pt">（+Mem0/Wiki 補位）</text>

  <!-- Leaf 4：Self-host / multi-provider -->
  <rect x="475" y="150" width="100" height="50" fill="#c3343e" rx="4"/>
  <text x="525" y="168" text-anchor="middle" fill="#fff" font-size="8pt" font-weight="700">Self-host /</text>
  <text x="525" y="182" text-anchor="middle" fill="#fff" font-size="8pt">multi-provider</text>
  <text x="525" y="194" text-anchor="middle" fill="#fff" font-size="7pt">→ Mem0/Zep/Letta</text>

  <!-- 註腳 -->
  <text x="290" y="222" text-anchor="middle" font-size="8pt" fill="#666" font-style="italic">「事實演變→Zep」「個人化→Mem0」「Autonomous→Letta」「離線→Cognee/MemPalace」</text>
  <text x="290" y="236" text-anchor="middle" font-size="8pt" fill="#666" font-style="italic">屬 Self-host/multi-provider 路徑下的細部分流，見場景對應速查表。</text>
  <text x="290" y="254" text-anchor="middle" font-size="8pt" fill="#999" font-style="italic">對 self-host 企業，Long Context 路線實質不可用，memory 框架仍是主體。</text>
</svg>

*圖 9：兩層 gate 決策樹——先看 session 規模決定要不要上 memory 框架，再看部署環境決定走 Claude 生態還是 multi-provider；最後才到 Mem0 / Zep / Letta 的具體場景分流。*

**場景對應速查**：

| 場景 | 第一推薦 | 第二選擇 | 何時不需要 memory 框架 |
|---|---|---|---|
...
**關鍵 reframe**：第三方 memory 框架（Mem0 / Zep / Letta）的市場空間，被 Long Context + Cache 從上方擠壓、被 Anthropic Memory tool 從側面擠壓，但在「self-host enterprise + 跨月跨年 + 多 agent autonomous」這三個 niche 仍是不可替代。**選 Mem0 / Zep 之前，先問自己這三條路是否都不適用**。
...
> **本節結論**
> - 13 種常見 anti-pattern 涵蓋：hallucination amplification、stale memory、cross-session contamination、cost runaway、cache 失效、GDPR 刪除權、資料主權
> - **最容易被低估的是 cost runaway 與 GDPR / sovereignty 風險**——extraction-heavy 派每次 LLM call 累積，月成本可能超預期 10×
> - LLM Wiki 的 client-side 實作有 ~50% silent failure 率，需從 async background 改成 sync manual checkpoint
> - 中文場景預設 embedding 退化是常見坑，換 BGE-zh / jina-zh 可立即改善 12-18 點 recall
...
| **Fact 衝突無解** | Mem0 / Zep 同主題抽出矛盾 fact | 加 conflict resolution policy（社群已有實驗方案）|
| **Cost runaway** | Write-time LLM call × user 數 × session 頻率 → 月成本爆炸；Mem0 extraction-heavy 派最易發生 | 設 budget alarm；高頻 user 改用 verbatim / Long Context；批次 write 而非 per-message |
| **Long Context cache 失效成本** | 每次 cache miss = 全 prompt 重算；超過 5min TTL 或 prompt 變動就 miss | sticky session；不要每 request 改 system prompt；長 prompt 拆成可獨立 cache 的 prefix + suffix |
| **GDPR / 用戶刪除權無法落地** | 用戶要求刪除個人資料，但 fact 已散落 vector / KG / summary 多處；很難證明「徹底刪除」| 設計時用統一 retention key；定期 audit；MemPalace 的 verbatim drawer + closet 反而較易 traceable |
| **Hosted memory 的資料主權風險** | Mem0 cloud / OpenAI Memory / Anthropic Memory tool 都把 user 資料存於第三方；regulated industry / sovereign 場景不可接受 | 預設用 self-host（Mem0 OSS / Cognee / MemPalace / Letta self-hosted）|
...
> **本節結論**
> - 9 個方向分兩層：**(a) 已有雛形可指**（Cognitive Core / Personal AI / AI Coding / 科研 Autoresearch）；**(b) 純推測有合理根據**（多 agent Wiki / 教育 / 企業 KM / Robotics / Wiki→Fine-tune Loop）
> - 最直接落地：**Anthropic Haiku 4.5 + Memory tool + Files API** 已是商業可用 Cognitive Core 雛形
> - 最高賭注：**Wiki → Fine-tune Consolidation Loop**（把記憶烤回權重的循環）——若走通，當前所有框架都是過渡產品
> - 12 個月最可能落地：個人 PKM、AI Coding Agent 跨 session、企業 KM 取代 Confluence

> 本節分兩層：(a) 已有雛形可指；(b) 純推測但有合理根據。

### §9.1 Cognitive Core 願景 — (a) 雛形
...
> **本節結論**
> - 12 條限制中最關鍵的三條：**Karpathy-centric 取材偏誤**（七處引用、非 peer-reviewed）／**benchmark 跨家口徑差異**（不可直接比）／**中文場景未實測**（業界估計）
> - 主管 / 採購 / 投資判斷請以本聲明為前提，**不要把報告當作「最終答案」**
> - 這個領域當前最好的姿態是「**持續關注 + 小型試點**」，而不是「選定一家 all-in」
...
10. **Karpathy-centric 取材偏誤**——本報告 §1.1 / §1.2 / §3.3 / §5.2 / §5.4 / §9.1 / §9.9 七處引用 Karpathy 的 LLM Wiki gist、System Prompt Learning X 推文、Cognitive Core podcast 訪談，作為「範式線」推論的主軸。**這些都是非 peer-reviewed、非 Anthropic / OpenAI / Google research roadmap 的個人主張**。Karpathy 是有號召力的研究者，他的觀點值得深思，但不應視為已驗證的工程藍圖。讀者應自行 weight：與當前主流 frontier lab（Anthropic / OpenAI / DeepMind）的官方研究路線對照，Karpathy 的賭注哪些被驗證、哪些仍在實證。報告已嘗試補入 Anthropic Memory tool / OpenAI Memory 等對照（§2.7-§2.8），但結構性偏誤無法完全消除
11. **Long Context + Cache 路線的成本 / 效能數字未實測**——§2.6 引用的 cache 折扣比例與 1M context 推論成本，均依官方文件估計；自部署開源模型 1M 落地的「品質衰退」「算力門檻」描述為業界常見估計，未自跑 benchmark
12. **Anthropic Memory tool 的具體 primitive 規格仍在演進**——§2.7 描述基於公開資料整理，實際 API 介面可能與本報告描述有出入；以 Anthropic 官方文件為準
