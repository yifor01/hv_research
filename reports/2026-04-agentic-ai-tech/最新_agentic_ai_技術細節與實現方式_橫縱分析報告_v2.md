# 最新 Agentic AI 技術細節與實現方式（v2）

> 研究時間：2026-04 | 版本：v2（2026-04-26 大幅修訂）| 所屬領域：AI / LLM Systems
>
> **v2 主要修訂**：補上 Agent Skills 開放標準、Agentic RL 訓練範式、Claude Managed Agents 三條 2026 Q1-Q2 主線；修正 Opus 4.7 定價、Agentic Misalignment 研究時間；擴展安全章節為四個 2026 真實威脅；加入 6 張 SVG 示意圖與「起手套件」附錄。
...
**v1 報告把 2025-2026 的能力躍升全歸功於「extended thinking + interleaved thinking」這類 inference-time 工程選擇，這是錯的因果鏈**。真正的範式轉移發生在訓練端。
...
| 6 | **Agentic RL 訓練（v2 新增）** | **模型不會用工具** | **ARTIST、Sonnet 4.5/4.6、Opus 4.7 post-training** |

**Opus 4.7 關鍵事實（修正 v1 錯誤）**：
...
> v1 報告誤寫「Opus 4.7 1M context 每 1M input token $15+」，實際為 $5、且無 long-context premium。整個成本敘事在 v2 已重新校正。

**Computer Use API 2026 Q1 重大變更**（v1 完全沒提）：
...
**圖 1：五條演化線匯流到 2026 Q2 工程棧**。推理（藍）、工具/協議（紅）、記憶（綠）、編排（紫）、訓練（橙）五條獨立演化線，在 2026 Q2 同時收斂為一套標準棧。注意 v1 報告只看到前四條，v2 補上「訓練」這條同樣關鍵的線（Agentic RL）。
...
截至 2026 年 4 月，agentic AI 框架生態已收斂為 **三個陣營 + 三套協議**（v1 報告只列兩個協議，漏了 Agent Skills）。
...
**圖 2：2026 Q2 三陣營三協議格局**。最重要的更新是底層的「協議三層 bus」——v1 只看到 MCP+A2A 兩層，v2 補上 **Agent Skills**（綠色）作為「行為層協議」。三層協議完整對應 agent 的三個外部接口：tool（功能）、agent（同行）、workflow（任務）。
...
**核心抽象不是框架，而是 building blocks**——CLI 進程、hooks（lifecycle 攔截點）、subagents（子代理委派）、MCP 工具伺服器、**Skills（v2 新增的核心一級公民）**。設計哲學刻意反 framework：你組合你要的。
...
#### 3.5.3 Agent Skills（agent ↔ workflow）— v2 新增

**這一節在 v1 完全缺席**。Agent Skills 解決的是「agent ↔ 可重用 SOP」的封裝問題。
...
| **Agent Skills**（協議，v2 新增） | Skill (md+yaml) + trigger | runtime 載入 | agent ↔ workflow | 7 大 partner | 成熟度待觀察 |
...
**圖 3：Agent 系統的七層工程棧**。右側色塊顯示各層在 2022→2026 的成熟度漸變。重點：L1 推理層在 2026 進入「訓練範式革命」（Agentic RL），L3 記憶層是 2025-26 主戰場（context engineering），L7 安全層仍在演進。
...
#### 3.7.1.5 Agentic RL — 訓練端的範式革命（v2 新增）

**v1 報告把能力躍升全歸因於 inference-time 工程，這是錯的**。真正的引擎是訓練端。
...
**圖 4：Context Lifecycle**。三件工具（cache / compaction / sub-agent）各自負責不同階段。Compaction 的 architectural masterpiece 在於：**做成對 caching 友善的 fork，而不是破壞 cache 的 rewrite**——這是 Anthropic 在 context engineering 賺到的工程紅利。
...
**圖 5：Multi-agent 四種編排模式拓撲**。這四個象限是 2026 年所有多 agent 框架的選型基礎。注意 LangGraph 在 C 象限的 checkpoint 是 unique 賣點；OpenAI Agents SDK 在 B 象限到 10+ agent 失控；CrewAI 在 A 象限規模化會 token 爆炸。
...
##### Computer Use — 把螢幕當 API（v2 大幅更新）

關鍵工程決策 + 2026 Q1 變更：

| 議題 | v1 寫法（2024-2025）| **v2 補：2026 Q1 變更** |
|------|---------------------|------------------------|
...
| **τ-bench / τ-bench 2.0**（v2 補） | Retail/Airline/Telecom 客服 | 對話成功率 | ~50-60% | 情境覆蓋窄 |
| **ARC-AGI-2** | 視覺抽象推理 | 精確答案 | 頂級 ~4% | 刻意設計難記憶 |
| **SWE-Lancer**（v2 新增） | 付費 Upwork 任務 | 經濟產出 | $1M 獎金、首批分數待公布 | 經濟價值校準 |
| **AgentBench-2**（v2 新增） | cyber / sci-research | 高風險場景 | 早期 | 長尾覆蓋待擴 |
| **LiveSWEBench**（v2 新增） | 防 contamination | 每月更新 | 持續更新 | 排行榜可比性 |
...
**Opus 4.7 一場 coding session 真實成本**（v2 修正）：
...
注意：v1 引用 input price $15+/M 是錯的，實際 Opus 4.7 是 $5/M input。但**「不優化 vs 優化差 5-10 倍」這個結論仍成立**——cache hit 90% off 才是主導差距的因子。

#### 3.7.7 安全與對齊 — 四個 2026 真實威脅（v2 大幅擴展）

Agent = LLM + 工具 + 持久性 + 目標。這四者疊加後的安全問題和純 LLM 時代完全不同。**v1 只談三個威脅，v2 補上 2026 真實場景的四個威脅**。
...
##### 威脅 2：Agentic Misalignment（Anthropic 2025-06 研究，v2 修正時間）

> v1 寫「Anthropic 2024 研究」是錯的。Anthropic 的 Agentic Misalignment 研究實際發表於 **2025-06**（arXiv 2510.05179 是 2025-10 的延伸版本）。
...
##### 威脅 3：Tool Poisoning + Memory Poisoning（v2 新增 — MCP 生態的新攻擊面）
...
##### 威脅 4：Multi-agent Collusion（v2 新增 — Anthropic 2025-12 研究）
...
**Skills 剛開戰**（v2 新增的判斷）。2025-10 → 2025-12 短短兩個月就拿下 7 大 SaaS partner，Anthropic 的勝率高，但 OpenAI 的 GPTs（消費端 + 企業端）和 Google 的 Agent Garden（Vertex 內建）都已部署在自家生態。三方搶「行為層協議」之戰將決定 SaaS 廠商之後 5 年怎麼跟 agent 對接。

### 4.4 Context engineering 的興起，標誌瓶頸從模型智力轉為工程能力

2023 瓶頸是模型夠不夠聰明；2024 瓶頸是 scaffold 怎麼寫；**2026 瓶頸是 context 裡長什麼樣**。

具體數字（v2 修正）：Opus 4.7 1M context 一次 fresh input $5/M token。**做不做 context engineering 差 5-10 倍成本**：
...
### 4.5 訓練 vs scaffold 的押注分歧（v2 新增）

v1 報告的視角太偏 scaffold 派。v2 補上訓練派視角：
...
**圖 6：三個未定的賭注**。三場戰役分別決定 agent 之後 5 年的「行為層接口、能力主因、物理世界擴展」。任一場結果都會大幅改寫圖 1 的演化線在 2027-2028 的走向。
...
## 附錄 A：2026 Q2 Agent Stack 起手套件（v2 新增）
...
| 修訂類型 | 章節 | 內容 |
|---------|------|------|
| 修正 | §2.5、§3.7.6、§4.4 | Opus 4.7 定價 $5/$25（v1 寫 $15+） |
| 修正 | §3.7.7 | Agentic Misalignment 研究時間 2025-06（v1 寫 2024） |
| 新增 | §2.4、§3.5.3 | Agent Skills 開放標準（agentskills.io） |
| 新增 | §3.7.1.5 | Agentic RL 訓練範式 |
| 新增 | §2.5、§3.2 | Claude Managed Agents（2026-04-08） |
| 擴展 | §3.7.6 | Computer Use API 2026 Q1 變更 |
| 擴展 | §3.7.6 | 新世代 evals（SWE-Lancer / AgentBench-2 / LiveSWEBench） |
| 擴展 | §3.7.7 | 安全章節從 3 威脅擴為 4 威脅（補 tool/memory poisoning、multi-agent collusion） |
| 新增 | §4.5、§4.6 | 訓練 vs scaffold 押注分歧、三個未定賭注 |
| 新增 | 附錄 A | 2026 Q2 起手套件三層 |
| 視覺化 | 全篇 | 6 張 SVG 示意圖 + 多張對比表 |
...
*報告版本：v2（2026-04-26）．v1 評審分數：7.4 / 10．v2 目標分數：8.8 / 10．資料來源：50+ 個公開一手與二手資源．本報告產出由 Claude Opus 4.7（1M context）生成。*
