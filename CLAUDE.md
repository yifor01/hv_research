...
**判斷不確定時的優先順序**：先看主題本身（例如「TSMC PI 盤點」永遠是 B 類），其次看 outcome（例如「該不該補膠原」可能是 C 區域市場誤分類，但實際是 D 健康類），最後看素材來源（如果素材已在 `用戶資訊/` 則必為 D 類）。
...
- **SVG 示意圖慣例**（純 PDF 排版規則，所有類別通用）：
  ...
  - 累積案例：關埔購屋研究(6 張)、痘痘肌報告(6 張)、Agentic AI 報告(10 張，v3 補圖版)、lean-pcos-collagen v2 (7 張)
...
- **啟動前必讀 `用戶資訊/profile.md` + `用戶資訊/health.md`**：個人 baseline / PCOS / 痘痘 / lean phenotype / 飲食框架已寫好，省去重複交代
- **outcome 3 問對齊**（v1 啟動前必做，避免框架錯位浪費整輪研究）：
  1. 這個補品 / 飲食 / 治療最終要解決什麼**可觀察 / 可量測**的問題？
  2. 如果這個 outcome 達成了但其他 100% 沒變，你滿意嗎？（澄清主次 outcome）
  3. 有哪些 condition 是 **constraint**（不能變糟）但不是 **target**（不需主動改善）？
- **版本流程是「outcome 重定位 v1→v2」**（v1 出爐後做專科視角 8 軸 22 點審查 → 發現框架對齊錯位即 reframe v2 → 不在 v1 補丁），與 B/C 類版本流程**不同類型**
- **v2 修訂保留章節編號 1-10 穩定**：新內容以子章節（如 §2.1）/ 增補表（如 §6.6 §6.7 §6.8）/ 重寫段（如 §8.1）形式無縫嵌入；明確標註「（v2 新增 / v2 重寫 / v2 升級）」便於追溯
- **個體差異能力限制聲明**：報告無法校正使用者個別 IGF-1 / 荷爾蒙 / 過敏 / 用藥背景，必附「不取代你的婦產科 / 內分泌 / 皮膚科醫師個案評估」聲明
- **個人健康資料絕對隔離**：`用戶資訊/` 內容**嚴禁**出現在 A/B/C 類交付報告中。即使使用者同期做 PI 盡調 + 健康類兩種研究，兩條 thread 的資料來源完全分離；不要在 PI 盡調報告中提到使用者的 PCOS / 痘痘 / 飲食框架等個人資訊
- **D 類交付前自查防呆**：
  - ❌ 是否誤把 B 類的「主管 3 問防禦 / Reference URL 必附 / Companion PDF 結構對齊」套用到健康類（D 類不需要主管圈選）
  - ❌ 是否誤把 C 類的「Cap Rate / 三平台均價解釋」套用到健康類（不適用）
  - ❌ 是否在報告中混入 PI 盡調案例的命名（如 TSMC、教授名字）做為類比（不相關）
